...
int threshold_decrypt_lisss(LweSample *ciphertext, const LweParams* params, 
                            std::vector<int> parties, int t, int p, double sd)
{

    // threshold decrypt ciphertext
    auto result_plaintext = thresholdDecrypt(ciphertext, params, parties, t, p, sd);
    return (result_plaintext > 0 ? 1 : 0);
}

// int threshold_decrypt_additive(LweSample *ciphertext, std::vector<std::vector<int32_t>> shares, 
//                                         const LweParams *params, int p, double sd)
// {
//     Torus32 axs = 0;
//     int n = params->n;
...
void PRFEval(vector<LweSample*> &enc_prf_output, const vector<std::vector<int>>& netlist, const vector<LweSample*>& enc_prf_msg, 
                                ...
                                const TFheGateBootstrappingCloudKeySet *bk, int t, int p, std::vector<int> parties, double sd) {
    ...
    for (int i = 0; i < netlist.size(); ++i) {
        ...
        if (netlist[i].front() == 2) { // XOR/AND operation
            ...
            auto result_msg1 = threshold_decrypt_lisss(temp_result1, params, parties, t, p, sd);
            auto result_msg2 = threshold_decrypt_lisss(temp_result1, params, parties, t, p, sd);
...
int main(int argc, char* argv[]) {
    ...
    //     shares[p-1][i] = sk->lwe_key->key[i] - sum;

    // }

    
    // LISSS secret sharing
    double bound = 0.0125;
    std::vector<int> parties(t);
    std::iota(parties.begin(), parties.end(), 1);

    clock_t begin_ss = clock();
    shareSecret(t, p, sk->lwe_key, params);
    clock_t end_ss = clock();
    double time_ss = ((double) end_ss - begin_ss)/CLOCKS_PER_SEC;
    cout << "Secret sharing time: " << time_ss*1000 << " ms"<< endl;
    ...
    PRFEval(enc_prf_output, prf_netlist, enc_prf_msg, enc_prf_key, prf_output_arr, 
                        params, bk, t, p, parties, bound);
    ...
    // Validity check of LISSS scheme
    int n_size = 1;
    std::vector<LweSample*> enc_msg1(n_size);

    for (int i = 0; i < n_size; ++i) {
        enc_msg1[i] = new_LweSample(params);
        pk.Encrypt(enc_msg1[i], rand() % 2);
    }

    std::vector<int> clear_output1(n_size);
    std::vector<int> clear_output2(n_size);

    for (int i = 0; i < n_size; ++i) {
        clock_t begin_plain = clock();

        clear_output1[i] = bootsSymDecrypt(enc_msg1[i], sk);

        clock_t end_plain = clock();
        double time_plain = ((double) end_plain - begin_plain)/CLOCKS_PER_SEC;

        clock_t begin_threshold = clock();

        clear_output2[i] = threshold_decrypt_lisss(enc_msg1[i], params, parties, t, p, bound);

        clock_t end_threshold = clock();
        double time_threshold = ((double) end_threshold - begin_threshold)/CLOCKS_PER_SEC;

        cout << "Plain decryption time: " << time_plain*1000 << " ms"<< endl;
        cout << "Threshold decryption time: " << time_threshold*1000 << " ms"<< endl;

        cout << "Plain decryption = "<< clear_output1[i] << " -> " << "Threshold decryption = "<< clear_output2[i] << endl;
    }
